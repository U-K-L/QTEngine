...
void NormalPass::RenderObjects(VkCommandBuffer commandBuffer, uint32_t imageIndex, uint32_t currentFrame, UnigmaCameraStruct* CameraMain)
{
    ...
    renderingObjects[0]->RenderBrush(*app, commandBuffer, imageIndex, currentFrame, graphicsPipeline, pipelineLayout, descriptorSets[currentFrame], voxelizer->meshingVertexBuffers[currentFrame % QTDoughApplication::MAX_FRAMES_IN_FLIGHT], voxelizer->readBackVertexCount, voxelizer->indirectDrawBuffer);
}